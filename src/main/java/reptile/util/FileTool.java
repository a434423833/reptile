...
public class FileTool {
    ...
    private static String getFileNameByUrl(String url, String contentType) {
        //去除 http://
        if (url == null) {
            return RandomValue.getNum(100000000, 99999999) + "";
        }
        url = url.substring(7);
        //text/html 类型
     ...
     * 生成目录
     */
    private static void mkdir(String fileLoad) {
        dirPath = fileLoad;
        if (dirPath == null) {
     ...
     */
    public static void saveToLocal(Page page, String fileLoad) {
        mkdir(fileLoad);
...
/*
    *//**
     ...
     * @param links
     *//*
    public static void saveToLocalS(Set<String> links, String fileLoad) {
        try {
            for (String url : links) {
                String fileName = getFileNameByUrl(url, "");
                mkdir(fileLoad);
                //Files.lines(Paths.get("D:\\jd.txt"), StandardCharsets.UTF_8).forEach(System.out::println);
                DataOutputStream out = new DataOutputStream(new FileOutputStream(new File(dirPath + "/" + fileName)));
                out.flush();
                out.close();
                System.out.println("文件：" + fileName + "已经被存储在" + dirPath);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
